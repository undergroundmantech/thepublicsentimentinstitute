#!/usr/bin/env python3
"""One-shot UI v2 port for app/page.tsx.

Operations:
  A) Replace ChartTip / SplitBar / SpreadBadge / ChartCard with v2 versions.
  B) Replace the entire HomePage `return (...);` block + trailing `}` with
     a v2 light/rounded layout that consumes the same computed data.
"""
import re
...
# ────────────────────────────────────────────────────────────────────────
# (A) Sub-components — find & replace by sentinel
# ────────────────────────────────────────────────────────────────────────
SUB_START = "// ─── Sub-components ───"
SUB_END   = "// ─── Live Results Config ───"
i = src.index(SUB_START)
j = src.index(SUB_END)

new_subs = '''// ─── Sub-components ───────────────────────────────────────────────────────────
...
'''
src = src[:i] + new_subs + src[j:]

# ────────────────────────────────────────────────────────────────────────
# (B) HomePage return block — replace everything from `return (` to EOF
# ────────────────────────────────────────────────────────────────────────
# Find the LAST `  return (` (HomePage's return) — anchor by indentation.
# We locate the `const issues = [...]` array end and the matching return.
ret_idx = src.rindex("  return (\n    <>\n      <style>")
src_head = src[:ret_idx]
    ...
    <>
      <style>{`
        ...
        @media (max-width: 1100px) {
          ...
          .hp-hero-left { grid-column: 1 / -1; }
        }
        @media (max-width: 720px) {
          .hp-hero { grid-template-columns: 1fr; gap: 16px; margin-bottom: 28px; }
        }
        ...
        .hp-hero-left {
          background:
            radial-gradient(ellipse 90% 100% at 0% 0%, rgba(124,58,237,0.10) 0%, transparent 60%),
            radial-gradient(ellipse 60% 60% at 100% 100%, rgba(37,99,235,0.06) 0%, transparent 60%),
            var(--panel);
          padding: 36px;
        }
        ...
        .hp-btn-primary:hover { background: #1a1d34; transform: translateY(-1px); text-decoration: none; color: #fff !important; }
        .hp-btn-ghost {
          background: var(--panel);
        ...
        .hp-cap-cta:hover { background: var(--purple-soft); transform: translateY(-1px); text-decoration: none; color: #fff !important; }
...
src = src_head + NEW_RETURN
P.write_text(src)
print(f"OK · new size = {len(src):,} bytes")
