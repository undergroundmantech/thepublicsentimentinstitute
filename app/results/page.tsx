...
function MapWithCountyTooltip({ svgText, regionResults }: { svgText: string; regionResults: RegionResult[] | Record<string, RegionResult> }) {
  ...
  // flip left/up if it would overflow the container
  ...
  setTooltip({
    ...
    y,
    title:
      currentRR?.region?.name ??
      (currentRR?.name ? titleCaseKey(currentRR.name) : prettyKey),
...
export default function March3FeaturedClient() {
  ...
  const stateLabels: Record<string, string> = { TX: "TEXAS", NC: "N. CAROLINA", AR: "ARKANSAS" };
  ...
  const timeStr = new Date(nowMs).toLocaleTimeString("en-US", { hour12: false, hour: "2-digit", minute: "2-digit", second: "2-digit" });
    ...
    <>
      <style>{`
        ...
        .res-panel {
          ...
          border: 1px solid var(--border);
          overflow: hidden;
          animation: res-fade-up 0.5s cubic-bezier(0.22,1,0.36,1) both;
        }
        .res-panel-header {
          display: flex;
          align-items: center;
        ...
        .res-overlay-card {
          ...
          box-shadow: 0 0 80px rgba(124,58,237,0.25), 0 30px 80px rgba(0,0,0,0.8);
        }
        .res-overlay-title {
          font-family: ui-monospace,'Courier New',monospace;
        ...
        .res-page-header::before {
          content: '';
          position: absolute;
          inset: 0;
          background:
            radial-gradient(ellipse 40% 80% at 0% 50%, rgba(230,57,70,0.04) 0%, transparent 70%),
            radial-gradient(ellipse 40% 80% at 100% 50%, rgba(37,99,235,0.05) 0%, transparent 70%);
          pointer-events: none;
        }
      ...
      <main className="res-root" style={{ minHeight: "100vh", background: "var(--background)", color: "var(--foreground)" }}>
        ...
        <div className="res-status-bar">
          <div className="res-status-bar-inner">
            ...
            <div className="res-note" style={{ letterSpacing: "0.22em", color: "rgba(255,255,255,0.22)" }}>{timeStr}</div>
          </div>
        </div>
          ...
          <div className="res-page-header-inner">
            <div style={{ display: "flex", flexWrap: "wrap", alignItems: "flex-end", justifyContent: "space-between", gap: "16px" }}>
              <div>
                ...
                <h1 className="res-page-title">
                  Election <em>Night</em>
                </h1>
              ...
              {racesForState.map((fr) => {
                ...
                return (
                  <button key={fr.id} className={`res-race-item ${isSelected ? "active" : ""}`} onClick={() => setSelectedId(fr.id)}>
                    <span className="res-race-label">{fr.label}</span>
