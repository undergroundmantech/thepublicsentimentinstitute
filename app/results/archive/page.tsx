...
import { DEDICATED_BOARDS, RACE_PAGES } from "./nightBoards";
    ...
    <main style={{ maxWidth: 900, margin: "0 auto", padding: "40px 16px 80px", fontFamily: "var(--font-body)" }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 12, color: "var(--muted2)", marginTop: 8, lineHeight: 1.6 }}>
          All tracked elections, organized by date. Open any night to read the board for it.
        </p>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
        {ELECTION_DATES.map(date => {
          ...
          // Every night has a board now: the three hand-built ones, or the
          // generic board at /results/archive/<date>.
          const dayHref = DEDICATED_BOARDS[date] ?? `/results/archive/${date}`;
          const hrefFor = (r: { id: number; slug: string }) =>
            RACE_PAGES.has(r.slug) ? getRaceUrl(r.id) ?? dayHref : dayHref;
          // Group by state
